...
public class MainKitchenActivityHelper {
		...
		try {
			...
			if (arrItem != null) {
				for (int i = 0; i < arrItem.length(); i++) {
					...
					try{
						voice.setType(results.getInt("type"));
					}catch (Exception e) {
					}
					lstVoice.add(voice);
				}
	...
	public static void ReadSound(Context mContext, MediaPlayer media, long time, final ArrayList<Voice> arr) {
		Log.d("sound2", "_____________________________");
		try {
	...
	// ======= Äá»c thÃ´ng bÃ¡o thÃªm, há»§y mÃ³n Äƒn =======================
